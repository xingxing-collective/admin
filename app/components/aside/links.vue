<template>
  <ul :class="ui.wrapper" v-bind="attrs">
    <component :is="'li'" v-for="(link, index) in links" :key="index" tag="li" :class="ui.container">
      <component
:is="link.children?.length ? Disclosure : 'div'" v-slot="slotProps"
        ...
        <component :is="link.children?.length ? DisclosureButton : 'div'" as="template">
          <UTooltip
class="flex" :popper="{ placement: 'right' }" :prevent="!link.tooltip" v-bind="link.tooltip"
            :ui="ui.tooltip">
            <ULink
v-slot="{ isActive }"
...
v-if="link.icon" :name="link.icon"
                ...
                <UAvatar
v-else-if="link.avatar" v-bind="{
                  size: ui.avatar.size,
                  ...link.avatar
                }" :class="[ui.avatar.base, link.avatarClass]" />
                <UChip
v-else-if="link.chip" v-bind="{
                  size: ui.chip.size,
...
v-if="link.badge" v-bind="{
        ...
        <Transition
v-bind="ui.transition" @enter="onEnter" @after-enter="onAfterEnter" @before-leave="onBeforeLeave"
          @leave="onLeave">
          <DisclosurePanel
v-if="link.children?.length && (slotProps?.open || link.collapsible === false)" static
            as="template">
