...
export default eventHandler(async (event) => {
  const { page, pageCount, name, email, location } = getQuery(event) as { page: number, pageCount: number, name?: string, email?: string, location?: string }

  const data = users
    .filter(x => x.name.search(name || '') > -1 && x.email.search(email || '') > -1 && x.location.search(location || '') > -1)
  return {
    data: data
